...
export function registerCronTools(server: McpServer, app: NewioAppForMcp, onToolCall?: ToolCallHook): void {
  server.registerTool(
    'schedule_cron',
    {
      description:
      ...
      inputSchema: {
        expression: z
          .string()
          .describe('Schedule expression. Examples: "every 23s", "every 45m", "at 2026-04-09T12:00:00Z"'),
  ...
  server.registerTool(
    'cancel_cron',
    {
      ...
      inputSchema: { cronId: z.string().describe('The cron job ID returned by schedule_cron') },
    },
    ({ cronId }) => {
